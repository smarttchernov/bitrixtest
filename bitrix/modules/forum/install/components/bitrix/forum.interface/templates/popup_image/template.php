...
elseif ($arParams['MODE'] == 'SHOW2IMAGES')
{
	...
	?><img style="border:none;" src="<?=$img["src"]?>" width="<?=$img["width"]?>" height="<?=$img["height"]?>" <?
...
else
{
	...
	ob_start();
	?><script><?
		?>if(window.addForumImagesShow){addForumImagesShow('<?=$id?>');}<?
		?>else{<?
			?>oForumForm=(!!oForumForm?oForumForm:{});<?
			?>if(!oForumForm.images_for_resize){<?
				?>oForumForm.images_for_resize=['<?=$id?>'];<?
				?>BX.loadScript('<?=CUtil::GetAdditionalFileURL('/bitrix/js/forum/popup_image.js')?>');<?
			?>}<?
			?>else<?
			?>{<?
				?>oForumForm.images_for_resize.push('<?=$id?>');<?
			?>}<?
		?>}<?
	?></script><?
