...
class Google extends Engine
{
	...
	{
		$queryResult = $this->query(self::SCOPE_BASE.self::SCOPE_FEED_SITES);
		if($queryResult->status == self::HTTP_STATUS_OK && strlen($queryResult->result) > 0)
		{
	...
	{
		$queryResult = $this->query($this->getSiteId($domain, $dir));
		if($queryResult->status == self::HTTP_STATUS_OK && strlen($queryResult->result) > 0)
		{
		...
		if($this->engineSettings['AUTH'])
		{
			...
			switch($method)
			{
				...
				case 'PUT':
					...
					$result = $http->query($method, $arUrl['host'], $arUrl['port'], $arUrl['path_query'], $data, $arUrl['proto'], 'application/atom+xml');
				break;
				case 'DELETE':
