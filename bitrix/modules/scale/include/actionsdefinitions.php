...
$actionsDefinitions = array(
	...
	"COPY_KEY_TO_SERVER" => array(
		...
		"USER_PARAMS" => array(
			"SERVER_IP" => array(
				"NAME" => Loc::getMessage("SCALE_ADEF_CHPASS_UP_IP"),
				"TYPE" => "STRING",
				"REQUIRED" => "Y",
			),
			"ROOT_PASSWD" => array(
	...
	"MYSQL_ADD_SLAVE_FIRST" => array(
		"START_COMMAND_TEMPLATE" => "sudo -u root /opt/webdir/bin/bx-mysql -a slave -s ##SERVER_PARAMS:hostname## -o json --mysql_password=##USER_PARAMS:MYSQL_PASS## --cluster_password=##CODE_PARAMS:CLUSTER_PASS## --replica_password=##CODE_PARAMS:REPLICA_PASS## ",
		...
		"USER_PARAMS" => array(
			"MYSQL_PASS" => array(
				"NAME" => Loc::getMessage("SCALE_ADEF_MYSQL_PASS"),
				"TYPE" => "PASSWORD"
			)
		),
		"CODE_PARAMS" => array(
			"CLUSTER_PASS" => '$l=20; $c="abcdefghiknrstyzABCDEFGHKNQRSTYZ1234567890"; $nc=strlen($c);  $s=""; for($i=0; $i<$l; $i++) {$s.=substr($c,rand(1, $nc) - 1, 1);} return $s;',
			"REPLICA_PASS" => '$l=20; $c="abcdefghiknrstyzABCDEFGHKNQRSTYZ1234567890"; $nc=strlen($c);  $s=""; for($i=0; $i<$l; $i++) {$s.=substr($c,rand(1, $nc) - 1, 1);} return $s;'
		),
		"CONDITION" => array(
			"COMMAND" => "sudo -u root /opt/webdir/bin/bx-mysql -a options -o json",
			"PARAMS" => array( "options-bxMysql:configure", "===", "'false'")
		)
	),
	...
	"MYSQL_ADD_SLAVE" => array(
		...
		"CONDITION" => array(
			"COMMAND" => "sudo -u root /opt/webdir/bin/bx-mysql -a options -o json",
			"PARAMS" => array( "options-bxMysql:configure", "===", "'true'")
		)
	),
	...
	"MEMCACHED_ADD_ROLE" => array(
		"START_COMMAND_TEMPLATE" => "sudo -u root /opt/webdir/bin/bx-mc -o json -a create j -s ##SERVER_PARAMS:hostname##",
	...
	"MEMCACHED_DEL_ROLE" => array(
		"START_COMMAND_TEMPLATE" => "sudo -u root /opt/webdir/bin/bx-mc -o json -a remove j -s ##SERVER_PARAMS:hostname##",
