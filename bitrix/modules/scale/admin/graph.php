...
if(\Bitrix\Scale\Helper::checkBxEnvVersion())
{
	if(\Bitrix\Scale\Monitoring::isEnabled())
	{
		...
		reset($serversList);
		$serverHostname = isset($_REQUEST["SERVER_HOSTNAME"]) ? $_REQUEST["SERVER_HOSTNAME"] : key($serversList);
		$monitoringCategoryId = isset($_REQUEST["GRAPH_CATEGORY"]) ? $_REQUEST["GRAPH_CATEGORY"] : "SYSTEM";
