...
if (!(($arParams['UPLOADER_TYPE'] == 'applet' && $arParams["VIEW_MODE"] == "applet") ||
		($arParams['UPLOADER_TYPE'] == 'flash' && $arParams["VIEW_MODE"] == "applet")) && is_object($arParams["bxu"]))
{
CJSCore::Init(array('uploader', 'canvas'));
$arParams["id"] = randString();
...
<div class="bxu-thumbnails bxu-thumbnails-start" id="bxuMain<?=$arParams["id"]?>"> <!-- bxu-thumbnails-loading bxu-thumbnails-start-->
	<div class="bxu-top-block">
		<label class="pg-top-bar-text" for="photo_album_id<?=$arParams["UPLOADER_ID"]?>"><?=GetMessage("P_TO_ALBUM")?>:</label>
			<select class="pg-select" name="photo_album_id" id="photo_album_id<?=$arParams["id"]?>" onclick="this.nextSibling.style.display=(this.value=='new'?'':'none');">
		...
		<div class="bxu-settings-block">
			<span class="bxu-settings-block-templates">
				<span class="bxu-templates-btn bxu-templates-btn-small" id="bxuReduced<?=$arParams["id"]?>" title="<?=GetMessage("MFU_SIMPLIFIED")?>"></span><?
				?><span id="bxuEnlarge<?=$arParams["id"]?>" class="bxu-templates-btn bxu-templates-btn-active bxu-templates-btn-big" title="<?=GetMessage("MFU_NORMAL")?>"></span>
			</span>
			<?/*?><span class="bxu-set-btn">Settings</span><?*/
				if ($arParams['UPLOADER_TYPE'] != 'form'):?>
				<span class="bxu-set-btn"><a class="bxiu-mode-link" href="<?= $arParams["MULTIPLE_FORM_URL"]?>"><?= GetMessage('P_SHOW_APPLET')?></a></span><?
				endif;?>
		</div>
	</div>
...
<?
	...
	else if ($arParams['UPLOADER_TYPE'] == 'flash' && $arParams["VIEW_MODE"] == "applet")   /*Show Flash uploader*/
	{
